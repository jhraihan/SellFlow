...
a Postgres database, the Django API (`shopflow-api`) and the React static site
(`shopflow-web`).
...
2. Render asks for the values marked `sync: false`. Enter the URLs the services will
   ...
   |---|---|---|
   | shopflow-api | `CORS_ALLOWED_ORIGINS` | `https://shopflow-web.onrender.com` |
   | shopflow-api | `FRONTEND_URL` | `https://shopflow-web.onrender.com` |
   | shopflow-web | `VITE_API_URL` | `https://shopflow-api.onrender.com/api/v1` |
...
taken, Render adds a suffix such as `shopflow-api-x7k2.onrender.com`. In that case, fix the
...
`DJANGO_SUPERUSER_FULL_NAME`) on shopflow-api, then redeploy. The build creates the account
...
| Secret | Value |
|---|---|
| `SHOPFLOW_API_URL` | `https://shopflow-api.onrender.com` (your real API URL, no `/api/v1`) |
| `SHOPFLOW_CRON_SECRET` | The `CRON_SECRET` value from shopflow-api's **Environment** page |
